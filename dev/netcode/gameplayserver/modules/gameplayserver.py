            ...
            if client:
                ...
                for item in items:
                    ...
                    if flag.lower() == "join":
                        ...
                        if (not self.gamestate.playerIsInGame(ticket)) and (not self.gamestate.playerNameIsInGame(username)):
                            self.gamestate.addPlayer(ticket, username)
                            client.send("join", 1)
                    ...
                    if flag.lower() == "txt":
                        ...
                        self.textEverybody(ticket, self.gamestate.getPlayerName(ticket), message)
    ...
    def cleanGamestate(self):
        ...
        for ticket in gamestate.contents["P"]:
            if not theater.hasSeat(ticket):
                playersToRemove.append(ticket)

        for ticket in playersToRemove:
            self.textEverybody(0, "Server", "%s has left the game."%(gamestate.getPlayerName(ticket)))
            del gamestate.contents["P"][ticket]
            print "    %s removed from gamestate." % (ticket)
