...
class Class:
	def dampify(self, v, res=10.0):
		f = (v * -1) * res
		return f

	# Basic omni-directional damping.
	def damp(self, obj, resistance=10.0):
		vel = obj.getVelocity()
		vx = vel[0]
		vy = vel[1]
		vz = vel[2]
		
		fx = self.dampify(vx, resistance)
		fy = self.dampify(vy, resistance)
		fz = self.dampify(vz, resistance)
		
		obj.applyForce([fx, fy, fz], 0)

	# Applies the damping to only X and Y axes.
	# Good for character physics.
	def dampXY(self, obj, resistance=10.0):
		vel = obj.getVelocity()
